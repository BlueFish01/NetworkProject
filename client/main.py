...
def SignPDF():
    ...
    if PdfPath :
        ...
        with open(PdfPath,'rb') as file_object:
            temp = file_object.read()
            AESencrypt(temp) 
            

        files=[
        ('PDF',(f'{time_stamp}.bin',open('./encrypted.bin','rb'),'application/octet-stream'))
        ]
        headers = {}
...
def VerifyThis(id):
    ...
    temp = base64.b64decode(datatemp[1])

    dataDict = datatemp[0]

    with open(f'./{dataDict["filePath"]}',"wb+") as file_object:
            file_object.write(temp)

    pdfBytes = AESdecrypt(f'./{dataDict["filePath"]}')
    
    os.remove(f'./{dataDict["filePath"]}')
    ...
    if(HashDataInt == DecryptedCertificate):
        ...
        with open(f'./Download/{dataDict["id"]}.pdf',"wb+") as file_object:
            file_object.write(pdfBytes)

        #send delete request via API

        DeleteInbox(dataDict["id"])
        ...
        showinfo(
            title='Error',
            message= "Can't Verify this Pdf, file is damaged or edited"
        )
...
class btn:
    def __init__(self,key,value):
        self.bt = Button(VerifyPage_Frame,text="From: "f'{value}'"\n id :"f'{key}',command=lambda: VerifyThis(self.key)).pack(pady=10)
        self.key = key
